package encoding

import "errors"

// Reference for EBCDIC (CP037/IBM037) tables:
// https://www-01.ibm.com/software/globalization/ccsid/ccsid037.html
// Table values sourced from IBM documentation and verified against moov-io/iso8583 and Wikipedia.

var (
	_ Encoder = (*ebcdic037Encoder)(nil)

	// EBCDIC037 is a stateless, reusable encoder for EBCDIC (CP037/IBM037). Safe for concurrent use.
	EBCDIC037 Encoder = ebcdic037Encoder{}
)

// Errors for encoding/decoding.
var (
	ErrDstTooShort   = errors.New("destination buffer too short")
	ErrInvalidASCII  = errors.New("input contains non-ASCII byte")
	ErrInvalidEBCDIC = errors.New("input contains invalid EBCDIC byte")
)

// ebcdic037Encoder implements Encoder for EBCDIC (CP037/IBM037).
type ebcdic037Encoder struct{}

// ebcdic037ToASCII and asciiToEBCDIC are lookup tables for fast conversion.
var ebcdic037ToASCII = [256]byte{
	// EBCDIC (CP037) to ASCII lookup table
	,
}
var asciiToEBCDIC037 = [256]byte{
	// ASCII to EBCDIC (CP037) lookup table
	,
}

// Encode converts ASCII bytes to EBCDIC (CP037/IBM037).
// Returns a new slice, or error if non-ASCII bytes are found.
func (ebcdic037Encoder) Encode(src []byte) ([]byte, error) {
	out := make([]byte, len(src))
	for i, b := range src {
		if b > 0x7F {
			return nil, ErrInvalidASCII
		}
		out[i] = asciiToEBCDIC037[b]
	}
	return out, nil
}

// Decode converts EBCDIC (CP037/IBM037) bytes to ASCII.
// Returns a new slice.
func (ebcdic037Encoder) Decode(src []byte) ([]byte, int, error) {
	out := make([]byte, len(src))
	for i, b := range src {
		out[i] = ebcdic037ToASCII[b]
	}
	return out, len(src), nil
}

func (ebcdic037Encoder) Name() string {
	return "EBCDIC-CP037"
}
